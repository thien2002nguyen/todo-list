...
export default function ModalAddNew({ closeModal, bookData }) {
    ...
    const [price, setPrice] = useState(bookData.price || 0);
    const [available, setAvailable] = useState(bookData.available || 0);
    ...
    const handleUpdate = async () => {
        ...
        try {
            await apiBook.putBook(bookUpdate);
            toast.warning("Đã cập nhật thông tin");
        } catch (error) {
